...
TestWholeMatrixTransforms := function(mat, scalar)
    local i, j, src, copy, srccopy, basedomain, eq, eqsrc;
    ...
    eqsrc := function()
        local i, j;
        for i in [1..NrRows(src)] do
            for j in [1..NrCols(src)] do
                if srccopy[i,j] <> src[i,j] then
                    return false;
                fi;
            od;
        od;
    ...
    if not eq() then Error("AddMatrix(", scalar, ") failure"); fi;
    if not eqsrc() then Error("AddMatrix source modified"); fi;

    AddMatrix(mat, src, scalar);
    AddMatrix(copy, srccopy, scalar);
    if not eq() then Error("AddMatrix(_,_,", scalar, ") failure"); fi;
    if not eqsrc() then Error("AddMatrix(_,_,scalar) source modified"); fi;

    MultMatrixLeft(mat, scalar);
    MultMatrixLeft(copy, scalar);
    if not eq() then Error("MultMatrixLeft(", scalar, ") failure"); fi;

    MultMatrixRight(mat, scalar);
    MultMatrixRight(copy, scalar);
    if not eq() then Error("MultMatrixRight(", scalar, ") failure"); fi;

    MultMatrix(mat, scalar);
    MultMatrix(copy, scalar);
    if not eq() then Error("MultMatrix(", scalar, ") failure"); fi;
end;
